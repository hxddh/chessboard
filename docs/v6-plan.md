...
> **状态：6.0.0 已发布（2026-09-19）。** §7 是落地记录，§8 是发布后复查发现的
> 问题与它们的去向。本文件承接 [`refactor-plan.md`](./refactor-plan.md)（2.0，已全部完成）
...
| Q2.5 胜率差 | 完成（验收按 §2 Q2.5 的原文：120 ms 下 `?!` ≥ 60%） | 完整的表是六格，不是一格。`measured.json` `winPctNoise` 与 `scanNoise` 同源对照，两档 movetime × 三个标记：<br>120 ms `?!` 39% → 67% · `?` 58% → 36% · `??` 86% → 71%<br>400 ms `?!` 53% → 48% · `?` 88% → 38% · `??` 75% → 83%<br>胜率差在六格里赢两格。验收只写了 120 ms 的 `?!`，那一格达标；其余不达标的格子当初没有写进这张表，现在补上。样本是 4 局 168 手，`?!` 事件两遍各 11 与 14 个，这个量级的重合率置信区间很宽，且加深到 400 ms 反而更不一致——6.1 把语料扩到 20 局以上重测（见 §8） |
...
| Q3.2 题库管线 | **部分** | `import-puzzles.mjs` + `puzzle-gate.mjs` 管线完成并有样本测试，但本机不可达 `database.lichess.org`，Lichess 题未入库。改走第二来源：`mine-puzzles.mjs` 引擎自弈挖题（900 局），过同一套门禁并经引擎复核，入库 958 题（一步杀 42 · 两步杀 27 · 三步杀 49 · 吃子 300 · 战术 540，其中捉双 125 · 牵制 106 · 串击 48 · 闪击 7 · 双将 10）。「每母题 ≥ 100 题」捉双与牵制已达；串击 / 闪击 / 双将在自弈里天然稀少，未达。**§5 的另一半验收「每 200 分评级段 ≥ 50 题」也未达，当初漏写**：1000-1199 段 42 题、1800-1999 段 43 题、2000-2199 段 6 题，六段里三段不足。出厂目标 2000 题，实发 168 + 958 = 1126 题 |
...
6.0.0 发布后对 §7 逐条复核代码，加上对 `persist.js` / `pgn-parser.js` /
`game-tree.js` / `engine.js` / `review.js` 五个模块的对抗性审计。下面每一条都
经过复现，全部在 6.1 修掉（`docs/v6-1-plan.md` 是施工账本）。

### 8.1 §7 说完成、实际没完成的

| 项 | §7 写的 | 实际 |
|---|---|---|
| Q1.3 引擎自愈 | 「超时 stop、两次 terminate 重建」 | **重建不了**。`init()` 交出 wasm 后把 `CHESS_SF_WASM_B64` 置空，之后任何一次 teardown 再 `init()`，`loadSources()` 因 b64 已空而不走第一个分支，又因 `sourcesPromise` 仍在而直接返回那个已兑现的 promise，脚本不再注入，`init()` 抛 `engine sources missing`。刻意留着「给重建用」的 `wasmBytes` 在抛错那行之后，永远够不着。而且两次罢工也凑不齐：`onProgress()` 在任何一次 `waitFor` 兑现时清零罢工数，而每次搜索前都有一次 `isready` 排空 |
| Q2.2 PGN 往返 | 「26 份语料往返」 | 实为 27 份，且都是自写的形状用例（`twic-header-set.pgn` 是歌剧之夜的着法配虚构棋手名），而 §2 Q2.2 的验收写的是「20 份**公开棋谱库样本**」。手写样本只能验证想到的形状 |
| Q2.2 `[FEN]` 校验 | 「`[FEN]` 导入走 `ChessEditor.validate`」（§2 原文） | 只接在手动「载入 FEN」对话框上，`importPgnText` 的主路径完全不校验。而且 `fromFen` 经 chess.js 建棋盘，多出来的王在 `validate()` 看到之前就被吃掉了 |
| Q1.1 存储 | 「`recover()` 以更新的一方为准」 | 四条丢档路径：镜像写抢在 `recover()` 前面、恢复后 1.2 秒窗口被 `beforeunload` 覆盖、`restoreAll()` 不看写入返回值、`.bak` 只写不读 |
| Q2.5 判语 | —— | 6.0 把 `verdictKey` 的输入换成了胜率精准度，却留着照厘兵曲线读的 90 / 75 两道坎，每个人被高看一档 |

### 8.2 §5 的验收里没有对过账的

§7 开头写「按 §2 的编号逐条对账」——对的是实现项，不是 §5 的验收项，于是有四条
无人认领，仓库里也找不到断言：首屏到可交互 < 1 s；走一步棋 `draw()` 恰一次；
预走在对手走完后 ≤ 1 帧；每 200 分评级段 ≥ 50 题。另有 `scripts/test-classics.mjs`
与 `scripts/test-mines.mjs` 写好了却没接进任何工作流，而发布说明称 test-classics
进了 `test:static`。

### 8.3 审计出的其余缺陷

PGN：`0-0+` 整局解析失败；空着 `--` / `Z0` 连 `splitGames` 一起炸；变着重复主线
着法会偷走主线（静默且往返稳定）；无 `[Result]` 头的棋局导出丢结果（D1 在新树路径
复发）；序列化会吐出超 80 列的行；注释里的 `}` 被改写成 `]`；无着变着里的注释漏到
下一个主线着法；`e.p.` 与图形化 SAN 致命。

引擎：teardown 不结算在飞的 `waitFor`，无限分析期间一次 teardown 会让
`exclusive()` 队列永久卡死；`waitFor` 不记世代，残留定时器会朝新 worker 记罢工；
持续分析的 `stop()` 在自己的搜索还没开始时就发出，会截断别人的搜索，而被截断的结果
按请求预算入了缓存；改 Hash 不清缓存。

复盘：`evalScalar` 把杀棋映射到 9500–9990，杀棋与厘兵之间的跳变被钳成满额漏着记进
ACPL，赢定的残局里快扫时有时无地报杀就会被记成两个大漏着。

学习：958 道挖出来的题只有 10 个不同的评级值（300 道全 1200，358 道全 1400），
`estimateRating` 是按类别基数加常数，评级编码的是类别不是难度，「按 ±150 出题」
实际等于「按类别出题」。

首屏：`bundle.js` 1.68 MB 同步加载，其中 ECO 462 KB、挖出来的题 160 KB、非当前
语言的课程与开局约 300 KB 首屏都用不上。

### 8.4 6.1 的处置（摘要，细节见 `v6-1-plan.md` §9）

§8.1 到 §8.3 的每一条都在 6.1 修掉了。三项结论值得留在这里，因为它们改写了
6.0 的说法：

**一、`?!` 在任何预算下都不可复现。** 语料从 4 局 168 手扩到 28 局 1320 手，
三档预算各扫两遍：

|  | 120 ms | 400 ms | 1200 ms |
|---|---|---|---|
| `?!` | 37% / 52% | 50% / 56% | 54% / 54% |
| `?` | 63% / 62% | 69% / 68% | 77% / 84% |
| `??` | 97% / 88% | 85% / 93% | 92% / 95% |

（厘兵 / 胜率）十倍搜索也没让 `?!` 变得可复现，两种口径都一样。所以 §2 Q2.5
设想的「跟着 movetime 调阈值」不成立，也不存在这个应用能拿出来的、能修好它
的预算。6.0 公布的 67% 是 11 与 14 个事件的小样本运气。结论写进 `review.js`：
`?!` 是这个应用唯一不敢背书的标注。

**二、评级段 8/9 达标，2200 段达不到。** 重新标定（实测的节点阶梯 + 首选次优
分差）后题量 958 → 1023，九段里八段到了 §5 要求的 50 题。2200–2399 段 27 题：
低水平自弈成批产出送子，难局面天生稀少，这个来源填不满它。没有把及格线改
低——`test-learning.mjs` 的 `BAND_SHORT` 按实际钉住，另有一条断言保证只有记录
在案的那一段可以短缺。

**三、§5 的四条验收，三条有了数字，一条不达标。** 首屏到可交互 252 ms（要求
< 1 s，达标）；预走在对手走完后恰好 1 帧（达标）；每 200 分段 ≥ 50 题（见上）；
**走一步棋 `draw()` 跑 2 次而不是 1 次**——两个调用点是 `gameMove()` 的 commit
与尾随 `sync()` 的 commitAll，§7 那句「commitAll 一次通知」没把第一个算进去。
e2e 同时断言 `<= 2` 与 `=== 2`，哪天变成 1 会当场失败，改的人顺手把这份账本
也对上。

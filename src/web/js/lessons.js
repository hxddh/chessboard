  ...
  export const CHESS_LESSONS = [
    ...
    },
    {
      ...
      tasks: [
        ...
        // 7.0: 这个包围圈原本画在 c1–e3，也就是有三个白兵站在第一横线上 ——
        // 一个兵永远不可能出现在自己的底线，这个局面是非法的。chess.js 照收，
        // Stockfish 18 也照算，而 **Stockfish 19 直接 abort 整个 wasm 模块**
        // （RuntimeError: unreachable），引擎自愈也救不回来。整体上移一格，
        // 图还是那张图，局面是合法的。
        { type: "stars", fen: "k7/8/8/8/2PPP3/2PNP3/2PPP3/K7 w - - 0 1", only: "n",
          prompt: "马被自家兵团团围住？没关系，它会跳！从包围圈里连踩 3 颗星",
          stars: ["f4", "e6", "c5"], solution: ["d3f4", "f4e6", "e6c5"] },
    ...
    },
    {
      ...
      tasks: [
        { type: "move", fen: "7k/8/8/3Q3r/8/8/8/K7 w - - 0 1", goal: "safe", only: "q",
    ...
    },
    {
      ...
      tasks: [
        ...
        { type: "move", fen: "7q/6k1/8/8/8/8/4K3/7R w - - 0 1", goal: "capture", target: "h8",
    ...
    },
    {
      ...
      tasks: [
        { type: "move", fen: "7k/8/8/3Q4/8/8/8/K7 w - - 0 1", goal: "check",
        ...
        { type: "move", fen: "4r2k/8/8/8/8/8/8/2B1K3 w - - 0 1", goal: "any",
          ...
          retry: "被将军时只有三条路：走开王（d1/d2/f1/f2）、拿象挡在 e 线上（Be3），或吃掉攻击子。别的走法界面不会让你走",
          solution: ["Be3"] },
    ...
    },
    {
      ...
      tasks: [
        { type: "move", fen: "4k3/P7/8/8/8/8/8/4K3 w - - 0 1", goal: "promote",
          ...
          retry: "这一课要的是升变本身 —— 走 a7–a8，然后在弹窗里挑一个子。走王不算",
          solution: ["a8=Q+"] },
      ],
    },
    {
    ...
    },
    {
      ...
      text: [
        ...
        "关键在**撑腰**：后贴上去自己也在王的攻击范围里，**必须有另一个子保护这个格子**，否则黑王一口吃掉，你白送一个后。这一课的撑腰角色是 h6 的象 —— 它从侧面死死盯住 g7。",
        "所以贴脸杀的检查清单只有两条：**后贴得上去吗？那一格有人保护吗？** 两条都是「是」，才是杀；只有第一条，那是送后。",
      ],
      tasks: [
        // 7.0：撑腰的象原本站 b2，盯着 a1–h8 整条长斜线。可那条斜线的尽头
        // 就是 h8 —— 象在撑住 g7 的同一瞬间也在将着黑王，而轮到白方走。
        // 这不是摆得不好，是这张图根本不可能合法：只要象从长斜线上保护 g7，
        // 它就一定同时攻击 h8。Stockfish 18 照算不误，**19 直接 abort 整个
        // wasm 模块**。改成 h6 的象从 f8–h6 这条斜线撑 g7，图的意思一字不变。
        { type: "move", fen: "7k/8/6QB/8/8/8/8/6K1 w - - 0 1", goal: "mate",
          prompt: "h6 的象看住了 g7 —— 把后贴上去",
          retry: "Qg7：后紧贴黑王，g7 由 h6 的象保护着，黑王吃不掉，h7、g8 又都被后封死",
          solution: ["Qg7#"] },
        { type: "move", fen: "7k/7p/6QB/8/8/8/8/6K1 w - - 0 1", goal: "mate",
          prompt: "这次黑王自己还有个 h7 兵堵着退路 —— 同样一步",
          retry: "Qg7：还是 h6 的象撑着 g7，而 h7 被黑方自己的兵占了",
          solution: ["Qg7#"] },
      ],
    },
    {
      ...
      tasks: [
        // 7.0：这两题原本把车摆在 h1 / a1，也就是在轮到白方走之前，车就已经
        // 沿着那条线将着黑王了 —— 局面非法，而 Stockfish 19 会 abort。车改为
        // 先站在第七横线上横过去，「马先就位、车最后进来将军」的顺序没变。
        { type: "move", fen: "7k/R7/5N2/8/8/8/8/6K1 w - - 0 1", goal: "mate",
          prompt: "f6 的马已经盖住了 g8 和 h7 —— 车沿第 7 横线横过去",
          retry: "Rh7：车将军，而 h7 有 f6 的马保护；黑王去 g8 也是马的火力范围",
          solution: ["Rh7#"] },
        { type: "move", fen: "k7/7R/2N5/8/8/8/8/6K1 w - - 0 1", goal: "mate",
    ...
    },
    {
      ...
      tasks: [
        { type: "tap", fen: "r2q1rk1/ppp3pp/4pn2/8/8/3NPN2/PPP3PP/R2Q1RK1 w - - 0 1",
          prompt: "黑方的 d 兵和 f 兵都不在了 —— 找出那个洞",
          steps: [
            { tip: "点击 e5：它左右两边的 d 兵、f 兵黑方都没有了，谁站上去都赶不走", squares: ["e5"] },
          ] },
        { type: "move", fen: "r2q1rk1/ppp3pp/4pn2/8/8/3NPN2/PPP3PP/R2Q1RK1 w - - 0 1",
          goal: "one-of", accept: ["Nde5", "Nfe5"],
          prompt: "把马跳到 e5 这个前哨上 —— 两匹马都够得着，挑一匹",
          retry: "d3 和 f3 的马各一步就能到 e5", solution: ["Nfe5"] },
      ],
    },
    {
      ...
      tasks: [
        { type: "tap", fen: "r3k2r/pp3ppp/8/3p4/8/2P2N2/PPP2PPP/R3K2R w KQkq - 0 1",
          prompt: "认一认这盘棋里的两种弱兵",
          steps: [
        ...
        { type: "move", fen: "r3k2r/pp3ppp/8/3p4/8/2P2N2/PPP2PPP/R3K2R w KQkq - 0 1",
          goal: "one-of", accept: ["Nd4", "c4"],
          prompt: "看懂了就该用上 —— 对付孤兵有两条路：占住它前面那个洞，或者上去逼它。走一步试试",
          retry: "两条路：Nd4 把马停在孤兵前面那个永远没人守的格，或者 c3–c4 顶上去逼这个孤兵表态",
          solution: ["Nd4"] },
      ],
    },
    {
      ...
      tasks: [
        ...
        { type: "move", fen: "6k1/5ppp/8/2P5/8/8/5PPP/6K1 w - - 0 1",
          goal: "one-of", accept: ["c6", "Kf1"],
          prompt: "让这个通路兵变强 —— 推它，或者先把王送过去给它开路，两样都对",
          retry: "推 c5–c6，或者走王往 c 兵那边靠（Kf1）", solution: ["c6"] },
      ],
    },
    {
      ...
      tasks: [
        ...
        { type: "move", fen: "3r2k1/ppp2ppp/8/8/8/8/PPP2PPP/4RRK1 w - - 0 1",
          goal: "one-of", accept: ["Rd1", "Re8+"],
          prompt: "后已经兑掉了，继续兑 —— 把车摆到 d 线上和黑车对脸，或者直接沿 e 线将过去逼它换",
          retry: "两条路：e1 的车走到 d1，或者 Re8+ 将军逼黑车吃", solution: ["Rd1"] },
      ],
    },
    {
      ...
      tasks: [
        ...
        { type: "move", fen: "r1bq1rk1/ppp2ppp/3p1n2/8/8/3B1N2/PPP2PPP/R1BQ1RK1 w - - 0 1",
          goal: "one-of", accept: ["Re1", "Ng5", "Qe2", "Qe1", "Qd2"],
          prompt: "两个子还不够 —— 再往黑王那边加一个子，凑够第三个。车、马、后都行",
          retry: "往王翼加人：Re1 把车调到空着的 e 线，Ng5 把马跳到课文说的老位置，或者 Qe2 / Qe1 / Qd2 把后调出来",
          solution: ["Re1"] },
      ],
    },
    // —— 第八部分 · 残局基础 ——
    ...
    // 不先讲这两条,drill-pawn 就只能靠试错。
    {
      ...
      tasks: [
        ...
        { type: "stars", fen: "k7/8/8/3K3p/8/8/8/8 w - - 0 1", only: "k",
          prompt: "换个方向看同一件事：黑兵在 h5，走 4 步到 h1；你的王站在方块的角上 d5 —— 沿对角线走过去，数一数也是 4 步",
          stars: ["e4", "f3", "g2", "h1"], solution: ["d5e4", "e4f3", "f3g2", "g2h1"] },
    ...
    // scripts/test-chess.mjs 里被引擎复核过(错色象靠自我对弈证明,不靠分数)。
    {
      ...
      tasks: [
        { type: "stars", fen: "7k/8/8/3KP3/8/8/8/8 w - - 0 1", only: "k",
          prompt: "e5 兵的关键格是它正前方那三格 —— 用王走一遍 d6、e6、f6：占住其中任何一个，这个兵都一定能升变",
          stars: ["d6", "e6", "f6"], solution: ["d5d6", "d6e6", "e6f6"] },
    ...
    },
    {
      ...
      tasks: [
        { type: "stars", fen: "k7/8/8/8/8/8/8/7K w - - 0 1", only: "k",
          prompt: "先把这一课的第一句话走一遍：王从角上 h1 出发，三步走到中心 e4 —— 残局里这往往就是第一步棋",
          stars: ["g2", "f3", "e4"], solution: ["h1g2", "g2f3", "f3e4"] },
    ...
    // —— 中级 · 战术组合 ——
    // 「吃子与价值」教的是单个母题；这一部分把两个母题接成一条线：先做一件
    // 事，再收另一件。每课一次点选（先看清局面里的关键子）加一次落子。
    // 24 个局面全部经 Stockfish 复核：课上要求的着法要么是引擎首选，要么
    // 与首选同级；一课题目从不教引擎会摇头的棋。
    {
      id: "tc-zwischenzug", part: "战术组合", title: "中间着：先将军，再吃回来",
      text: [
        "对方刚吃了你一个子，你的本能是**立刻吃回来**。可有时候在吃回来之前，还能先插进一步「非应不可」的棋 —— 通常是将军 —— 这一步叫**中间着**。",
        "这里黑象刚吃到 f3，你的 c4 象同时被 b5 兵攻击。直接 gxf3 吃回，黑就 bxc4 拿走你的象。先 Bxf7+ 将军：象反正保不住，换成一个兵**外加一次将军**，黑王被迫应将，你再从容 gxf3。",
        "口诀：**被吃之后别急着吃回，先看有没有带将军的棋**。",
      ],
      tasks: [
        { type: "tap", fen: "r2qr1k1/p1p2ppp/2n5/1p6/2B5/2NP1b2/PPP3PP/R2QR1K1 w - - 0 1", prompt: "先看清两处：哪个子刚被吃，哪个子正要被吃", steps: [
          { tip: "点黑象落脚的 f3 —— 它刚吃掉你的兵，你随时能 gxf3 吃回", squares: ["f3"] },
          { tip: "点你的 c4 象 —— 它正被 b5 兵攻击，一步之内就会丢", squares: ["c4"] },
        ] },
        { type: "move", fen: "r2qr1k1/p1p2ppp/2n5/1p6/2B5/2NP1b2/PPP3PP/R2QR1K1 w - - 0 1", goal: "check",
          prompt: "别急着吃回 f3 —— 先用那个保不住的象将军",
          retry: "c4 象反正要丢，让它临走前将一军：Bxf7+", solution: ["Bxf7+"] },
      ],
    },
    {
      id: "tc-smother", part: "战术组合", title: "闷杀实战：先逼进角，再堵门",
      text: [
        "「杀型积木」里你见过闷杀的形状。实战里它很少现成摆好 —— 通常要先**用将军把王逼进角落**，再让对方自己的子堵住出口。",
        "这个局面黑王已经在 h8，g8 车和 g7、h7 兵把它围得严严实实。这正是马最喜欢的形状：一步 Nf7，无处可逃。",
        "识别信号：**王在角上、旁边全是自己人**。看见这个，就找马。",
      ],
      tasks: [
        { type: "tap", fen: "6rk/6pp/8/6N1/8/8/6PP/6K1 w - - 0 1", prompt: "找出把黑王闷在角落的三个「自己人」", steps: [
          { tip: "点 g8 —— 黑车占着王唯一的横向出口", squares: ["g8"] },
          { tip: "点 g7 和 h7 —— 两个兵把前面封死", squares: ["g7", "h7"] },
        ] },
        { type: "move", fen: "6rk/6pp/8/6N1/8/8/6PP/6K1 w - - 0 1", goal: "mate",
          prompt: "一步将死", retry: "马从 g5 跳到哪一格能将军 h8？", solution: ["Nf7#"] },
      ],
    },
    {
      id: "tc-decoy", part: "战术组合", title: "引入 + 捉双：先把王骗过去",
      text: [
        "捉双要成立，两个目标得站在马的「日」字上。目标不在？**把它请过去**。",
        "这里黑王在 g8、后在 d8，马在 e5 —— 从 f7 跳能同时攻到 h8 和 d8。所以先 Rh8+！黑王只能吃车（f8 被 a3 象封着），一到 h8，Nxf7+ 就叉住王和后。",
        "一车换一后加一兵，这就是**弃子引入**：牺牲的子把对方的王送到捉双的位置上。",
      ],
      tasks: [
        { type: "tap", fen: "r2q2k1/pp3pp1/8/4N3/8/B7/PP3PP1/R5KR w - - 0 1", prompt: "先找到捉双的位置", steps: [
          { tip: "点 f7 —— 马跳到这里能同时攻击 h8 和 d8", squares: ["f7"] },
          { tip: "点 h8 —— 黑王现在不在这里，得把它请过来", squares: ["h8"] },
        ] },
        { type: "move", fen: "r2q2k1/pp3pp1/8/4N3/8/B7/PP3PP1/R5KR w - - 0 1", goal: "check",
          prompt: "用车把黑王引到 h8", retry: "把车送到 h8 将军 —— 黑王除了吃它别无选择", solution: ["Rh8+"] },
      ],
    },
    {
      id: "tc-clearance", part: "战术组合", title: "让路：自己的子挡了自己的线",
      text: [
        "你的车在 d1，对方的车在 d8，中间只隔着**你自己的马**。这时该做的不是换车，而是让马**带着将军**离开 d 线。",
        "Nf6+ 或 Ne7+：马走开的同时将军，黑方必须应将 —— 没时间救 d8 的车。下一步 Rxd8。",
        "让路走法的要点：挪开的那步棋**本身要带威胁**，否则对方就有时间处理。",
      ],
      tasks: [
        { type: "tap", fen: "3r2k1/5ppp/8/3N4/8/8/5PPP/3R2K1 w - - 0 1", prompt: "找出挡路的子和它挡住的目标", steps: [
          { tip: "点 d5 的马 —— 它站在你的车和对方的车之间", squares: ["d5"] },
          { tip: "点 d8 的黑车 —— 马一走开它就暴露在你的车前", squares: ["d8"] },
        ] },
        { type: "move", fen: "3r2k1/5ppp/8/3N4/8/8/5PPP/3R2K1 w - - 0 1", goal: "one-of", accept: ["Nf6+", "Ne7+"],
          prompt: "让马带着将军离开 d 线", retry: "马有两格能将军 —— f6 或 e7，哪个都行", solution: ["Nf6+"] },
      ],
    },
    {
      id: "tc-skewer", part: "战术组合", title: "串击实战：王在前面，后在后面",
      text: [
        "串击是「先攻前面的大子，逼它让开，再吃后面的」。最强的串击是**王在前面**：王必须应将，后面的子就没人管。",
        "黑王 e5、黑后 e8 在同一条直线上，你的车只要上 e 线将军，王一走，后就是你的。",
        "找串击的方法：**看对方的王和后有没有站在同一条线上**，再看自己有没有子能占到那条线。",
      ],
      tasks: [
        { type: "tap", fen: "4q3/8/8/4k3/8/8/6PP/R5K1 w - - 0 1", prompt: "找到同一条线上的两个目标", steps: [
          { tip: "点 e5 的黑王", squares: ["e5"] },
          { tip: "点 e8 的黑后 —— 和王在同一条 e 线上", squares: ["e8"] },
        ] },
        { type: "move", fen: "4q3/8/8/4k3/8/8/6PP/R5K1 w - - 0 1", goal: "check",
          prompt: "用车串击王和后", retry: "车上 e 线：Re1+", solution: ["Re1+"] },
      ],
    },
    {
      id: "tc-interference", part: "战术组合", title: "拦截实战：切断守卫的视线",
      text: [
        "黑后在 b7 远远守着 g7，你的后已经到了 h6 —— 只差把这条第 7 横线**切断**。",
        "d5 的马一动就是闪将（c4 象照到 g8）。跳到 e7 或 c7，马正好站在 b7 和 g7 之间：黑王只能退 h8，然后 Qxg7 将死 —— 黑后隔着马，救不到。Nf6+ 也行：马撑住 h7，Qxh7 将死。",
        "拦截的本质：**找到守卫的那条线，把一个子插进去**，最好还带着将军。",
      ],
      tasks: [
        { type: "tap", fen: "5rk1/pq4pp/7Q/3N4/2B5/8/1B4PP/6K1 w - - 0 1", prompt: "找到守卫和它守的格子", steps: [
          { tip: "点 b7 的黑后 —— 它沿着第 7 横线守着 g7", squares: ["b7"] },
          { tip: "点 g7 —— 你想在这里将死，只要切断黑后的视线", squares: ["g7"] },
        ] },
        { type: "move", fen: "5rk1/pq4pp/7Q/3N4/2B5/8/1B4PP/6K1 w - - 0 1", goal: "one-of", accept: ["Ne7+", "Nc7+", "Nf6+"],
          prompt: "用马闪将，同时切断第 7 横线", retry: "马跳到第 7 横线上（e7 或 c7）就挡住了黑后 —— 或者 f6 撑住 h7", solution: ["Ne7+"] },
      ],
    },
    {
      id: "tc-overload", part: "战术组合", title: "过载实战：先兑掉一个，再拿另一个",
      text: [
        "黑后在 d7 同时守着 e8 车和 d5 马。**一个子干不了两份活**：你先在一处兑掉它，它就顾不上另一处。",
        "Rxe8+ 逼黑后吃回 —— 它一到 e8，d5 的马就没人守了，Qxd5。",
        "过载的解法总是同一个顺序：**先动被守的两个目标中「带将军的那个」**，再收另一个。",
      ],
      tasks: [
        { type: "tap", fen: "4r1k1/pp1q1ppp/8/3n4/8/1Q5P/PP3PP1/4R1K1 w - - 0 1", prompt: "找到过载的子和它守的两个目标", steps: [
          { tip: "点 d7 的黑后 —— 它一个子守着两个地方", squares: ["d7"] },
          { tip: "点它守的两个目标：e8 车和 d5 马", squares: ["e8", "d5"] },
        ] },
        { type: "move", fen: "4r1k1/pp1q1ppp/8/3n4/8/1Q5P/PP3PP1/4R1K1 w - - 0 1", goal: "capture", target: "e8",
          prompt: "先在 e8 兑掉，把黑后从 d5 的守卫岗位上拽开", retry: "Rxe8+ —— 黑后被迫吃回，d5 就空了", solution: ["Rxe8+"] },
      ],
    },
    {
      id: "tc-backrank", part: "战术组合", title: "底线杀 + 升变：车先冲上去",
      text: [
        "你的兵已经到了 c7，前面 c8 被黑车盯着。直接升变会被吃 —— 但你有车。",
        "Rd8+！黑车必须吃（王没地方去），然后 cxd8=Q+ —— 兵吃车升后，还是将军。一车换一车加一个后。",
        "升变前先问：**升变格被谁守着？能不能先用一步将军把守卫换掉？**",
      ],
      tasks: [
        { type: "tap", fen: "2r3k1/2P2ppp/8/8/8/8/6PP/3R2K1 w - - 0 1", prompt: "找到快升变的兵和挡路的守卫", steps: [
          { tip: "点 c7 的兵 —— 只差一步升变", squares: ["c7"] },
          { tip: "点 c8 的黑车 —— 它守着升变格", squares: ["c8"] },
        ] },
        { type: "move", fen: "2r3k1/2P2ppp/8/8/8/8/6PP/3R2K1 w - - 0 1", goal: "check",
          prompt: "用车将军，逼黑车离开 c8", retry: "Rd8+ —— 黑车吃了它，你的兵就吃回去升后", solution: ["Rd8+"] },
      ],
    },
    // —— 中级 · 典型残局 ——
    {
      id: "eg-cutoff", part: "典型残局", title: "切断：不让王靠近你的兵",
      text: [
        "车兵残局第一课不是推兵，是**把对方的王关在门外**。车横在一条横线上，对方的王就永远过不来。",
        "这里黑王在 e8，你的车上到 h7 之后，第 7 横线整条归你 —— 黑王只能在最后一排徘徊，你的王和兵慢慢推上去。",
        "记住：**先切断，再推兵**。切得越远，王越无能为力。",
      ],
      tasks: [
        { type: "tap", fen: "4k3/8/8/8/3PK3/8/8/7R w - - 0 1", prompt: "先看要关住谁", steps: [
          { tip: "点 e8 的黑王 —— 它想过来堵你的兵", squares: ["e8"] },
          { tip: "点 h7 —— 车站在这里，第 7 横线就成了黑王过不去的墙", squares: ["h7"] },
        ] },
        { type: "move", fen: "4k3/8/8/8/3PK3/8/8/7R w - - 0 1", goal: "one-of", accept: ["Rh7"],
          prompt: "用车切断第 7 横线", retry: "车上 h7", solution: ["Rh7"] },
      ],
    },
    {
      id: "eg-qvp", part: "典型残局", title: "后对七线兵：将军，逼王站到兵前",
      text: [
        "对方的兵到了第 7 横线，你的后要赢它，靠的是一个节奏：**不停将军，直到黑王被迫站到兵的前面**（e1），那一刻兵动不了，你的王走近一步。",
        "然后再重复：将军、逼王挡兵、王走近 —— 直到你的王也来到跟前吃掉兵。",
        "现在就开始：随便一步能将到 d2 王的棋都是好的起手。",
      ],
      tasks: [
        { type: "tap", fen: "1Q6/8/6K1/8/8/8/3kp3/8 w - - 0 1", prompt: "先看清楚兵和它的升变格", steps: [
          { tip: "点 e2 的黑兵 —— 只差一步", squares: ["e2"] },
          { tip: "点 e1 —— 你要把黑王逼到这格上挡住自己的兵", squares: ["e1"] },
        ] },
        { type: "move", fen: "1Q6/8/6K1/8/8/8/3kp3/8 w - - 0 1", goal: "one-of", accept: ["Qb2+", "Qb4+", "Qd6+", "Qf4+"],
          prompt: "用后将军，开始逼王", retry: "从 b8 出发，哪几格能将到 d2 的王？b2、b4、d6、f4 都行", solution: ["Qb2+"] },
      ],
    },
    {
      id: "eg-nvrp", part: "典型残局", title: "马守边兵：钻进角落也能和",
      text: [
        "只剩一个马对一个边兵，马守得住吗？守得住 —— 但要知道一个怪招。",
        "黑王 b2 正要赶走 b3 的马，然后 a1 升后。Na1！马钻进升变格：黑王吃它（Kxa1）之后，你的王 Kc2，黑方**没有棋可走**，逼和。黑王不吃，你的马就守在那里。",
        "残局里不要只想「保住子」——**能把局面走成逼和的子，比活着的子更值钱**。",
      ],
      tasks: [
        { type: "tap", fen: "8/8/8/8/8/1N6/pk6/3K4 w - - 0 1", prompt: "先看兵要去哪", steps: [
          { tip: "点 a1 —— 黑兵的升变格，也是你的马要去的格", squares: ["a1"] },
          { tip: "点 b3 的马 —— 黑王正要赶它", squares: ["b3"] },
        ] },
        { type: "move", fen: "8/8/8/8/8/1N6/pk6/3K4 w - - 0 1", goal: "one-of", accept: ["Na1"],
          prompt: "唯一能守和的一步", retry: "把马跳进 a1 —— 被吃也没关系，那是逼和", solution: ["Na1"] },
      ],
    },
    {
      id: "eg-rook7", part: "典型残局", title: "车占第 7 横线：吃兵，困王",
      text: [
        "残局里车最好的位置之一是**对方的第 7 横线**：那里通常摆着一排没动过的兵，而对方的王被压在底线。",
        "Rd7 之后 b7 兵就在你嘴边，黑王也被封在第 8 横线。对方要防你就得把车拉回来做被动的守卫。",
        "看见空开的直线，先想：**能不能从这条线冲到第 7 横线？**",
      ],
      tasks: [
        { type: "tap", fen: "5rk1/pp3ppp/8/8/8/8/PP3PPP/3R2K1 w - - 0 1", prompt: "先看第 7 横线上有什么", steps: [
          { tip: "点 b7 —— 车上第 7 横线后第一个目标", squares: ["b7"] },
          { tip: "点 d7 —— 车的落点", squares: ["d7"] },
        ] },
        { type: "move", fen: "5rk1/pp3ppp/8/8/8/8/PP3PPP/3R2K1 w - - 0 1", goal: "one-of", accept: ["Rd7"],
          prompt: "车占第 7 横线", retry: "沿开放的 d 线冲到 d7", solution: ["Rd7"] },
      ],
    },
    {
      id: "eg-kingact", part: "典型残局", title: "王要出动：残局里它是主力",
      text: [
        "中局里王要躲，残局里王要**冲到前面**。棋盘上没了后和大子，王就是最强的进攻单位。",
        "这个王兵残局双方兵数相同，赢家是**王先到中心的那一方**。第一步就是把王从 g1 挪出来，向 e3、d4 走。",
        "口诀：**大子一换光，王就往中间走**。",
      ],
      tasks: [
        { type: "tap", fen: "6k1/p5pp/1p6/2p5/2P5/1P6/P5PP/6K1 w - - 0 1", prompt: "看清起点和目的地", steps: [
          { tip: "点 g1 的白王 —— 它还躲在角落里", squares: ["g1"] },
          { tip: "点 d4 —— 王要去的中心格，从那里两翼都够得着", squares: ["d4"] },
        ] },
        { type: "move", fen: "6k1/p5pp/1p6/2p5/2P5/1P6/P5PP/6K1 w - - 0 1", goal: "one-of", accept: ["Kf2", "Kf1"],
          prompt: "让王迈出第一步，向中心走", retry: "Kf2 最直接（Kf1 也算）", solution: ["Kf2"] },
      ],
    },
    {
      id: "eg-bvn", part: "典型残局", title: "象对马：象能从远处困住马",
      text: [
        "棋盘开阔、两翼都有兵时，**象强过马**：象一步管一条长斜线，马要走好几步才能换一翼。",
        "更妙的是象能**困住**马。h8 的黑马只有 g6 和 f7 两个出口，它们在同一条斜线上 —— 象到 h5 一格就全封死。然后你的王慢慢走过去吃它。",
        "看见对方的马在边角上，先数它有几个出口，再看**一个象能不能同时盯住它们**。",
      ],
      tasks: [
        { type: "tap", fen: "7n/1k6/p7/8/3K4/P7/4B3/8 w - - 0 1", prompt: "先数马的出口", steps: [
          { tip: "点 h8 的黑马", squares: ["h8"] },
          { tip: "点它仅有的两个出口 g6 和 f7 —— 注意它们在同一条斜线上", squares: ["g6", "f7"] },
        ] },
        { type: "move", fen: "7n/1k6/p7/8/3K4/P7/4B3/8 w - - 0 1", goal: "one-of", accept: ["Bh5"],
          prompt: "用象一步封死马的两个出口", retry: "哪一格同时照到 g6 和 f7？h5", solution: ["Bh5"] },
      ],
    },
    {
      id: "eg-rvminor", part: "典型残局", title: "车对马：马离王越远越危险",
      text: [
        "单车对单马通常和棋 —— **只要马待在自己王的身边**。马一旦被赶到远处的角落，车和王合力就能困住它。",
        "a1 的黑马只有 b3 和 c2 两个出口。Rb2！车守 b 线管住 b3，第 2 横线管住 c2 —— 马一步也动不了，下一步就被吃。",
        "用马防守时记住：**别让马离开王**；用车进攻时记住：**先困住，再吃**。",
      ],
      tasks: [
        { type: "tap", fen: "k7/8/8/8/8/3K4/7R/n7 w - - 0 1", prompt: "先数马的出口", steps: [
          { tip: "点 a1 的黑马 —— 离自己的王太远了", squares: ["a1"] },
          { tip: "点它的两个出口 b3 和 c2", squares: ["b3", "c2"] },
        ] },
        { type: "move", fen: "k7/8/8/8/8/3K4/7R/n7 w - - 0 1", goal: "one-of", accept: ["Rb2"],
          prompt: "用车一步封死马的两个出口", retry: "哪一格同时管到 b3 和 c2？b2 —— 竖着管 b3，横着管 c2", solution: ["Rb2"] },
      ],
    },
    {
      id: "eg-queen", part: "典型残局", title: "后残局：将军顺手吃兵",
      text: [
        "后残局里最实用的技术是**带将军的吃兵**：先将军，王一躲，后就顺路吃掉一个没人守的兵。对方全程没有还手的时间。",
        "黑方 a7、h7 两个兵都没人守。Qh5+ 或 Qa4+，黑王一躲，下一步就吃兵。",
        "后残局找棋的顺序：**先找将军，再看将军后能顺路吃到什么**。",
      ],
      tasks: [
        { type: "tap", fen: "4k3/p6p/8/8/8/8/5PPP/3Q2K1 w - - 0 1", prompt: "找出没人守的兵", steps: [
          { tip: "点 h7 的黑兵", squares: ["h7"] },
          { tip: "点 a7 的黑兵 —— 两个都没人守", squares: ["a7"] },
        ] },
        { type: "move", fen: "4k3/p6p/8/8/8/8/5PPP/3Q2K1 w - - 0 1", goal: "one-of", accept: ["Qh5+", "Qa4+"],
          prompt: "将军，并且让后顺路盯上一个兵", retry: "Qh5+ 盯 h7，Qa4+ 盯 a7 —— 哪个都行", solution: ["Qh5+"] },
      ],
    },
    // —— 中级 · 兵型与计划 ——
    {
      id: "ps-iqp", part: "兵型与计划", title: "孤兵：要么突破，要么被围",
      text: [
        "d4 兵两边的 c 线、e 线都没有自己的兵 —— 这是**孤兵**。它守不住，但它换来的是空间和活跃的子力。",
        "有孤兵的一方只有一个计划：**在被围死之前突破**。d4–d5！开线、开斜线，让子力全部活起来。",
        "对方的计划正相反：牢牢占住 d5 格（在孤兵前面放一个马），然后慢慢围攻它。",
      ],
      tasks: [
        { type: "tap", fen: "r2qr1k1/pp1nbppp/2n1p3/8/3P4/2NB1N2/PPQ2PPP/R2R2K1 w - - 0 1", prompt: "先认出孤兵和它的突破格", steps: [
          { tip: "点 d4 —— 两边都没有自己的兵，这是孤兵", squares: ["d4"] },
          { tip: "点 d5 —— 它的突破格；对方若在这里放个马，孤兵就被封死了", squares: ["d5"] },
        ] },
        { type: "move", fen: "r2qr1k1/pp1nbppp/2n1p3/8/3P4/2NB1N2/PPQ2PPP/R2R2K1 w - - 0 1", goal: "one-of", accept: ["d5"],
          prompt: "趁 d5 还没被封住，突破", retry: "推兵：d5", solution: ["d5"] },
      ],
    },
    {
      id: "ps-hanging", part: "兵型与计划", title: "悬兵：c4 + d4 并肩站",
      text: [
        "c4、d4 两个兵并排站着，两侧都没有自己的兵 —— 这叫**悬兵**。它们并肩时控制着 b5、c5、d5、e5 一整排格子，很有力。",
        "悬兵的弱点是**被迫动一个**：一动就露出空档。所以主动方要在对方施压之前自己选时机推进：d5！打开局面。",
        "遇到对方的悬兵，就用子力压它，逼它动；自己有悬兵，就趁子力活跃时**主动推**。",
      ],
      tasks: [
        { type: "tap", fen: "2rr2k1/pb1nqppp/1p2pn2/8/2PP4/P1N2N2/1B2QPPP/3RR1K1 w - - 0 1", prompt: "先认出悬兵", steps: [
          { tip: "点并肩站着的两个兵：c4 和 d4", squares: ["c4", "d4"] },
          { tip: "点 d5 —— 突破格", squares: ["d5"] },
        ] },
        { type: "move", fen: "2rr2k1/pb1nqppp/1p2pn2/8/2PP4/P1N2N2/1B2QPPP/3RR1K1 w - - 0 1", goal: "one-of", accept: ["d5"],
          prompt: "趁子力活跃，主动推进", retry: "d5 —— 打开 b2 象和 e 线", solution: ["d5"] },
      ],
    },
    {
      id: "ps-doubled", part: "兵型与计划", title: "叠兵：两个兵，一份活",
      text: [
        "c6 和 c7 两个兵叠在同一条线上，这叫**叠兵**。前面的挡住后面的，后面的守不了前面的 —— 两个兵只顶一个用，而且后面那个（c7）永远没人守。",
        "黑虽多一个兵，但你的车能沿 d 线冲到第 7 横线：Rd7 直接吃到 c7 头上。",
        "评估兵多兵少时，**叠兵、孤兵都要打折**。",
      ],
      tasks: [
        { type: "tap", fen: "r5k1/p1p2ppp/2p2b2/8/8/5N2/PP3PPP/3R2K1 w - - 0 1", prompt: "先认出叠兵", steps: [
          { tip: "点叠在一起的两个黑兵 c6、c7", squares: ["c6", "c7"] },
          { tip: "点 d7 —— 车到这里就压住了后面那个兵", squares: ["d7"] },
        ] },
        { type: "move", fen: "r5k1/p1p2ppp/2p2b2/8/8/5N2/PP3PPP/3R2K1 w - - 0 1", goal: "one-of", accept: ["Rd7"],
          prompt: "车上第 7 横线盯住 c7", retry: "Rd7", solution: ["Rd7"] },
      ],
    },
    {
      id: "ps-minority", part: "兵型与计划", title: "少数兵进攻：用两个兵打三个",
      text: [
        "后翼你只有 a、b 两个兵，对方有 a、b、c 三个 —— 反而是你去进攻，这叫**少数兵进攻**。目标不是吃兵，而是**制造弱点**。",
        "推 b4–b5！兑掉之后对方要么留下一个孤兵 c6，要么留下一个落后兵 —— 你的车和马整个中局都有事可做。",
        "这个计划在「卡尔斯巴德结构」里几乎每局都要用，值得记住。",
      ],
      tasks: [
        { type: "tap", fen: "3rr1k1/pp1nqppp/2p2n2/3p4/1P1P4/P1N1PN2/5PPP/1QR2RK1 w - - 0 1", prompt: "先认出进攻的兵和它的目标", steps: [
          { tip: "点 b4 —— 少数一方的先锋", squares: ["b4"] },
          { tip: "点 c6 —— 兑掉后它会变成孤兵或落后兵", squares: ["c6"] },
        ] },
        { type: "move", fen: "3rr1k1/pp1nqppp/2p2n2/3p4/1P1P4/P1N1PN2/5PPP/1QR2RK1 w - - 0 1", goal: "one-of", accept: ["b5"],
          prompt: "开始少数兵进攻", retry: "b5 —— 逼 c6 表态", solution: ["b5"] },
      ],
    },
    {
      id: "ps-chain", part: "兵型与计划", title: "兵链：打它的底座",
      text: [
        "黑兵 e5、d6 斜着连成一条**兵链**：e5 靠 d6 守，d6 靠 c7 守。链子最结实的地方是头（e5），最脆的地方是**底座**。",
        "所以不要去撞头，去撬底座：c4–c5 打 d6。d6 一松，整条链子跟着松。",
        "对着兵链找计划时只问一句：**哪个兵是底座，我用哪个兵去碰它？**",
      ],
      tasks: [
        { type: "tap", fen: "r2q1rk1/ppp1bppp/3p1n2/3Pp3/2P1P3/2N1B3/PP3PPP/2RQ1RK1 w - - 0 1", prompt: "先认出兵链和它的底座", steps: [
          { tip: "点链头 e5 —— 别去撞它", squares: ["e5"] },
          { tip: "点底座 d6 —— 你要撬的地方", squares: ["d6"] },
        ] },
        { type: "move", fen: "r2q1rk1/ppp1bppp/3p1n2/3Pp3/2P1P3/2N1B3/PP3PPP/2RQ1RK1 w - - 0 1", goal: "one-of", accept: ["c5", "b4"],
          prompt: "用兵撬底座（直接 c5，或先 b4 备好再推）", retry: "c5 打 d6 —— 或者 b4 为它做准备", solution: ["c5"] },
      ],
    },
    {
      id: "ps-bishops", part: "兵型与计划", title: "好象与坏象：兑掉对方的好象",
      text: [
        "象走斜线，所以**站在和自己兵同色格上的象是坏象** —— 被自己的兵挡着。黑方 c8 象前面是 d5、e6 两个浅格兵，它几乎动不了。",
        "e7 象则是黑方的**好象**，它走深格，正对着你的 d4、e5。你的 g5 象刚好也是深格象 —— Bxe7 把它换掉，黑方只剩下那个被兵堵死的坏象。",
        "兑子原则：**兑掉对方的好子，留下他的坏子**。",
      ],
      tasks: [
        { type: "tap", fen: "2b3k1/p3bppp/4p3/3pP1B1/3P4/8/P3BPPP/6K1 w - - 0 1", prompt: "先分清好象和坏象", steps: [
          { tip: "点 c8 —— 黑方的坏象，被 d5、e6 兵堵着", squares: ["c8"] },
          { tip: "点 e7 —— 黑方的好象，你要把它换掉", squares: ["e7"] },
        ] },
        { type: "move", fen: "2b3k1/p3bppp/4p3/3pP1B1/3P4/8/P3BPPP/6K1 w - - 0 1", goal: "capture", target: "e7",
          prompt: "兑掉黑方的好象", retry: "Bxe7", solution: ["Bxe7"] },
      ],
    },
    {
      id: "ps-space", part: "兵型与计划", title: "空间优势：别急着换子",
      text: [
        "c4 + e4 两个兵把 d5 格牢牢钳住，黑方的子挤在后三排 —— 这就是**空间优势**（这个结构叫「马洛奇钳制」）。",
        "有空间的一方最怕**换子**：子越少，对方越不觉得挤。所以 Nxc6 是反着来；正确的是把马退到 c2 或 b3，或者用 f3 加固 e4，保持钳制。",
        "空间多的时候记住：**留着子，慢慢挤**。",
      ],
      tasks: [
        { type: "tap", fen: "r2q1rk1/pp1bppbp/2np1np1/8/2PNP3/2N1B3/PP2BPPP/2RQ1RK1 w - - 0 1", prompt: "先认出钳制", steps: [
          { tip: "点 c4 和 e4 —— 两个兵合力封住 d5", squares: ["c4", "e4"] },
          { tip: "点 d5 —— 黑方最想推 d5 解放自己，现在推不出来", squares: ["d5"] },
        ] },
        { type: "move", fen: "r2q1rk1/pp1bppbp/2np1np1/8/2PNP3/2N1B3/PP2BPPP/2RQ1RK1 w - - 0 1", goal: "one-of", accept: ["Nc2", "Nb3", "f3"],
          prompt: "保持钳制：别兑马，把它退下来或加固中心", retry: "Nc2、Nb3 退马，或 f3 加固 e4 —— 别 Nxc6", solution: ["Nc2"] },
      ],
    },
    {
      id: "ps-break", part: "兵型与计划", title: "兵的突破：撞掉对方的支点",
      text: [
        "黑方的 e5 兵是它整个阵地的**支点**：d6 象靠它站稳，中心靠它顶住。",
        "你不能吃它（fxe5 只会让 d6 象吃回来更活跃），但你可以**从旁边撞**：f4–f5！封住黑方的象和后，把 e5 变成一个孤零零的目标。",
        "兵的突破不总是为了吃兵 —— 常常是为了**改变格局**：换一个对自己有利的兵型。",
      ],
      tasks: [
        { type: "tap", fen: "3q1rk1/p4ppp/3b4/4p3/4PP2/3B4/P5PP/3Q1RK1 w - - 0 1", prompt: "先认出对方的支点", steps: [
          { tip: "点 e5 —— 黑方阵地的支点", squares: ["e5"] },
          { tip: "点 f5 —— 你的兵要去的格", squares: ["f5"] },
        ] },
        { type: "move", fen: "3q1rk1/p4ppp/3b4/4p3/4PP2/3B4/P5PP/3Q1RK1 w - - 0 1", goal: "one-of", accept: ["f5"],
          prompt: "从旁边撞：推 f 兵", retry: "f5 —— 不吃 e5，把它晾在那里", solution: ["f5"] },
      ],
    },
  ];

...
| 操作 | 说明 |
|------|------|
| **教学模式** | 侧栏「模式 → 教学」：**零基础 32 课**互动课程 —— 认识棋盘摆法 → 六种棋子走法+王的禁区(**每个棋子多套练习:绕障碍/吃子路线/精确落点**) → 子力价值/吃子保护/救受攻的子/捉双/**串击/闪击** → 将军应将(走/挡/**吃**)/**牵制**/将死/逼和/易位/吃过路兵/升变/和棋方式/记谱法 → **杀型积木(双车阶梯杀/闷杀/后车绞杀)** → 开局三原则 + **学者杀完整引导局(含防守要点)** → **王兵残局(护送升变)**·后杀王·车杀王实战（**先热身再实战**;引擎陪练用**新手档**会主动犯错,可悔棋/提示）；连错两次自动标答案,「演示」按钮可**随时重看解法**,进度可**重置**,毕业一键进入人机·新手 |
| **做题练习** | 侧栏「模式 → 做题」：**一步杀 / 两步杀 / 三步杀 / 吃子 / 战术母题 / 开局 / 复习**七类题库(58 题)（底线杀、闷杀、肩章杀、阿拉伯杀、升变杀、弃后引杀、双车赶王、**后王/车王逼角、升变三步杀**…）；答错回退并**说明黑方如何化解**;「答案」画正解箭头;杀王题接受任何可强制将死的替代解;吃子题选净得子力最多的一吃;战术母题(**串击/闪将/牵制/捉双**)强制得子;开局题执白照谱背 38 条主流线路;**答错或看答案的题自动进「复习」,做对即移出**；进度持久 |
| 走子 | **点击**或**拖拽**均可（拖动时棋子跟随指针）;走子有**滑动动画** |
| 提和 / 判和 | 「提和」双人双方确认即和,人机由引擎按局面评估接受/拒绝；**「判和」按 FIDE 声明和棋**——同一局面三次重复或 50 回合无吃子无动兵时亮起,五次重复 / 75 回合自动判和 |
| 人机对弈 | 默认与 **Stockfish 18** 对弈；侧栏可选难度（**新手·会主动犯错** · 入门 Elo 1320 · 进阶 1700 · 困难 2200 · 极限满强度）与执子（白/黑，选黑自动翻转视角） |
...
| 分析 | 「分析」快扫(120ms/步) / **「精析」深评(400ms/步)**：局势曲线 + `?!` `?` `??` 失着标注 + **引擎主变**（跟随复盘位置）；「重下」从任意一手继续对弈；人机模式可开**失着提醒**,走出严重失误立刻提示悔棋重想 |
| 复盘 | 着法列表（SAN）点击跳转；← → / Home / End；● 回到最新 |
| 棋谱 | 复制 / 导出 / **粘贴导入 / 打开文件 / 拖入窗口导入** PGN（导出含**标准七标签、TimeControl/Termination 与结果记号**；支持**从指定局面开始**的 `[FEN]` 棋谱,复盘/分析/重下均正确）；复制当前局面 FEN |
| 翻转 | **F** / 顶栏「翻转」切换黑白视角 |
| 悔棋 / 新局 | **Z** / **N**（有棋时确认）；人机模式悔棋会同时收回引擎回着 |
| 侧栏 | **Tab** 开关；主题 木/夜/日/纸 · 音效开关 |
| 存档 | 自动保存（含棋钟余时与认输状态），重开恢复 |
| **成就** | 侧栏「成就」：12 枚徽章从教学/做题/对局进度自动解锁(规则通关、战术之眼、屠龙、身经百战…);**一经解锁永久保留** |
...
「新手」用 `Skill Level` 低档实现（UCI_Elo 下限 1320 对零基础仍太强，低 Skill Level 会主动犯错）。

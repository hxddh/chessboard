...
| 操作 | 说明 |
|------|------|
| **教学模式** | 侧栏「模式 → 教学」：**零基础 22 课**互动课程 —— 认识棋盘摆法 → 六种棋子走法+王的禁区（吃星任务）→ 子力价值与吃子保护 → 将军/将死/逼和/易位/吃过路兵/升变/和棋方式/记谱法 → **开局三原则** → 后杀王·车杀王实战（引擎陪练,可悔棋/提示）；连错两次自动标出答案,进度自动保存,毕业一键进入人机·入门 |
| **做题练习** | 侧栏「模式 → 做题」：**一步杀 / 两步杀**经典杀型题库（底线杀、闷杀、肩章杀、弃后引杀…）；答错即回退,「答案」画正解箭头；两步杀接受任何**可强制将死**的替代解,黑方自动走最强防守；进度持久 |
...
- **v0.8**：题库扩容（三步杀、吃子战术）、拖拽走子、分析显示引擎主变
- 之后：人机模式计时、开局练习（照谱走主流开局）
...
src/web/
  ...
  js/lessons.js    # 零基础教学课程 22 课（单测逐课校验 FEN/解法/目标）
  js/puzzles.js    # 战术题库:一步杀/两步杀(单测用求解器证明强制杀)

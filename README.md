...
| **快捷键速查** | **?** 打开完整快捷键表(三语);macOS/Windows 另有**原生菜单**:对局(新局/悔棋/提示/翻转)、视图(侧栏/上一手/下一手)、帮助(快捷键),配 ⌘/Ctrl 组合键 —— 桌面用户第一眼会去找的地方不再是空的。**提示是 ⌘⇧H 而不是 ⌘H**:AppKit 先装系统的 app 菜单,其「隐藏应用」占着 ⌘H 且按菜单顺序先命中,1.10–1.17 这一项一直是死的,按下去还会把窗口藏起来(页面内直接按 `h` 不受影响)。**菜单本身 1.10–1.21 也是死的**:`app.zon` 填了、页面接了,但 `main.zig` 一直传 `.menus = &.{}`(非空空切片),`resolvedMenus` 的 `orelse` 永远走不到 `fromManifest()` —— 和 1.18 的 `close_policy` 同一种病,短路点在调用点而不在 runner |
...
- **审计**：[v1.21 代码与产品审计](docs/V1.21-AUDIT.md)（原生菜单通电、内容缺口、学习闭环）

...
| 人机对弈 | 默认与 **Stockfish 18** 对弈；侧栏可选难度（**新手 · 休闲**两档是浅层 MultiPV 抽样的手工削弱档,**用「只会不一步送子」的机器人对下 32 盘量出来:对新手得分率 56%、对休闲 27%、对 Elo 1320 的入门档几乎为零** —— 1.18 之前新手档的 `worstBias` 是 0.6(十个候选里挑最差的那个,六成的着法),同一个机器人能打出 81%,而它上面一格直接就是 1320 · 入门 Elo 1320 · 进阶 1700 · 困难 2200 · 极限满强度）与执子（白/黑，选黑自动翻转视角）；档位强度由 `scripts/test-strength.mjs` 实测把关 |
...
| 分析 | 「分析」快扫(120ms/步) / **「精析」深评(400ms/步)**,**运行中可随时「停止」并保留已算结果**：局势曲线 + `?!` `?` `??` 失着标注 + **引擎主变** + **双方准确率**（跟随复盘位置）；**形势条**——把曲线在当前这一手上读成一根不用看坐标轴的条（复盘限定：对局中不出现，也就不会变成答案表；纯读已算好的数，不占用引擎）；**「该走什么」箭头**——复盘停在一手被标了 `?!` `?` `??` 的着法上时，棋盘直接画出引擎会走的那一步；「重下」从任意一手继续对弈；人机模式可开**失着提醒**（静态预筛,仅在可能丢子时才占用引擎）|
...
src/web/
  ...
  js/i18n.js       # 界面文案字典 zh-CN / en / ja 各 608 条(单测校验键完整、无漏译、

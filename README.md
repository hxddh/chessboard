...
| 操作 | 说明 |
|------|------|
| **教学模式** | 侧栏「模式 → 教学」：**零基础 36 课**互动课程 —— 认识棋盘摆法 → 六种棋子走法+王的禁区(**每个棋子多套练习:绕障碍/吃子路线/精确落点**) → 子力价值/吃子保护/救受攻的子/捉双/**串击/闪击** → 将军应将(走/挡/**吃**)/**牵制**/将死/逼和/易位/吃过路兵/升变/和棋方式/记谱法 → **杀型积木(双车阶梯杀/闷杀/后车绞杀)** → 开局三原则 + **学者杀完整引导局(含防守要点)** → **王兵残局(护送升变)**·后杀王·车杀王·**双象杀王**·**卢塞纳(车兵搭桥取胜)**·**菲利多尔(车兵守和,和棋即成功)**·**后对车**实战（**先热身再实战**;引擎陪练用**新手档**会主动犯错,可悔棋/提示）；连错两次自动标答案,「演示」按钮可**随时重看解法**,进度可**重置**,毕业一键进入人机·新手 |
| **做题练习** | 侧栏「模式 → 做题」：**一步杀 / 两步杀 / 三步杀 / 吃子 / 战术母题 / 开局 / 复习**七类题库(58 题)（底线杀、闷杀、肩章杀、阿拉伯杀、升变杀、弃后引杀、双车赶王、**后王/车王逼角、升变三步杀**…）；答错回退并**说明黑方如何化解**;「答案」画正解箭头;杀王题接受任何可强制将死的替代解;吃子题选净得子力最多的一吃;战术母题(**串击/闪将/牵制/捉双**)强制得子;**难度分级(简单/中等/较难)** 由解法长度、子力密度与首着是否安静着自动推导,可按难度筛选;开局题执白照谱背 38 条主流线路,**每条附思路讲解**;**答错或看答案的题自动进「复习」,做对即移出**；进度持久 |
...
| 侧栏 | **Tab** 开关；主题 木/夜/日/纸 · **界面语言 中文/English**（界面、状态、提示与播报全部本地化；**教学前 9 课已英文化**,其余课程与题库内容仍为中文）· 音效开关 |
| 存档 | 自动保存（含棋钟余时与认输状态），重开恢复 |
| **成就 / 统计** | 侧栏「成就」：12 枚徽章从教学/做题/对局进度自动解锁,**一经解锁永久保留**；「统计」按难度记录胜负和,分析过的对局还会记录**准确率** |
...
- **后续**：教学剩余课程与题库内容英文化、自适应出题、更多开局体系讲解
...
src/web/
  ...
  js/lessons.js    # 零基础教学课程 36 课（单测逐课校验 FEN/解法/目标）
  js/puzzles.js    # 题库 58 题:杀王/吃子/战术母题(求解器证明强制)/开局
  ...
  js/i18n.js       # 界面文案字典 zh-CN / en(单测校验键完整、无漏译、tooltip 全接线)
  js/lessons-en.js # 教学课文英文试点(仅文案;局面与解法仍只来自 lessons.js)
